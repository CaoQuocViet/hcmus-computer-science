'use strict';

/** @type {import('sequelize-cli').Migration} */
module.exports = {
  async up (queryInterface, Sequelize) {
    await queryInterface.bulkInsert('Laptops', [
      {
        LaptopID: '12768',
        BrandID: 6,
        CategoryID: 3,
        Picture: '/Assets/data/laptops/01.jpg',
        ModelName: 'LG Gram 2023 16Z90RS',
        ScreenSize: 16.0,
        OperatingSystem: 'Windows 11',
        ReleaseYear: 2023,
        Description: 'Laptop siêu nhẹ với thời lượng pin ấn tượng',
        Discount: 12000000,
        DiscountStartDate: new Date('2025-01-15T00:00:00Z'),
        DiscountEndDate: new Date('2025-02-15T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-01-12T09:15:00Z'),
        updatedAt: new Date('2025-01-12T09:15:00Z')
      },
      {
        LaptopID: '16354',
        BrandID: 3,
        CategoryID: 2,
        Picture: '/Assets/data/laptops/02.jpg',
        ModelName: 'Lenovo Legion Pro 5 16IRX9 2024',
        ScreenSize: 16.0,
        OperatingSystem: 'Windows 11',
        ReleaseYear: 2024,
        Description: 'Laptop gaming cao cấp với hiệu năng mạnh mẽ',
        Discount: 12000000,
        DiscountStartDate: new Date('2025-01-20T00:00:00Z'),
        DiscountEndDate: new Date('2025-02-20T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-01-13T10:30:00Z'),
        updatedAt: new Date('2025-01-13T10:30:00Z')
      },
      {
        LaptopID: '18351',
        BrandID: 3,
        CategoryID: 3,
        Picture: '/Assets/data/laptops/03.jpg',
        ModelName: 'Lenovo Yoga 7 2-in-1 14IML9',
        ScreenSize: 14.0,
        OperatingSystem: 'Windows 11',
        ReleaseYear: 2024,
        Description: 'Laptop 2-in-1 linh hoạt với màn hình cảm ứng',
        Discount: 13000000,
        DiscountStartDate: new Date('2025-01-18T00:00:00Z'),
        DiscountEndDate: new Date('2025-02-18T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-01-14T11:45:00Z'),
        updatedAt: new Date('2025-01-14T11:45:00Z')
      },
      {
        LaptopID: '18950',
        BrandID: 2,
        CategoryID: 2,
        Picture: '/Assets/data/laptops/04.jpg',
        ModelName: 'Asus TUF Gaming A16 Advantage Edition FA617NT',
        ScreenSize: 16.0,
        OperatingSystem: 'Windows 11',
        ReleaseYear: 2024,
        Description: 'Laptop gaming bền bỉ với card đồ họa AMD',
        Discount: 13000000,
        DiscountStartDate: new Date('2025-01-15T00:00:00Z'),
        DiscountEndDate: new Date('2025-02-15T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-01-14T14:20:00Z'),
        updatedAt: new Date('2025-01-14T14:20:00Z')
      },
      {
        LaptopID: '19325',
        BrandID: 1,
        CategoryID: 2,
        Picture: '/Assets/data/laptops/05.jpg',
        ModelName: 'Dell Alienware X14 R2',
        ScreenSize: 14.0,
        OperatingSystem: 'Windows 11',
        ReleaseYear: 2024,
        Description: 'Laptop gaming mỏng nhẹ với thiết kế độc đáo',
        Discount: 14000000,
        DiscountStartDate: new Date('2025-01-25T00:00:00Z'),
        DiscountEndDate: new Date('2025-02-25T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-01-15T09:10:00Z'),
        updatedAt: new Date('2025-01-15T09:10:00Z')
      },
      {
        LaptopID: '19927',
        BrandID: 2,
        CategoryID: 2,
        Picture: '/Assets/data/laptops/06.jpg',
        ModelName: 'Asus ROG Strix G16 G614 Per Key RGB',
        ScreenSize: 16.0,
        OperatingSystem: 'Windows 11',
        ReleaseYear: 2024,
        Description: 'Laptop gaming cao cấp với bàn phím RGB từng phím',
        Discount: 14000000,
        DiscountStartDate: new Date('2025-01-28T00:00:00Z'),
        DiscountEndDate: new Date('2025-02-28T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-01-16T10:30:00Z'),
        updatedAt: new Date('2025-01-16T10:30:00Z')
      },
      {
        LaptopID: '20225',
        BrandID: 2,
        CategoryID: 2,
        Picture: '/Assets/data/laptops/07.jpg',
        ModelName: 'Asus ROG Zephyrus G14 GA403UV',
        ScreenSize: 14.0,
        OperatingSystem: 'Windows 11',
        ReleaseYear: 2024,
        Description: 'Laptop gaming nhỏ gọn với hiệu năng cao',
        Discount: 15000000,
        DiscountStartDate: new Date('2025-02-01T00:00:00Z'),
        DiscountEndDate: new Date('2025-03-01T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-01-18T13:45:00Z'),
        updatedAt: new Date('2025-01-18T13:45:00Z')
      },
      {
        LaptopID: '20269',
        BrandID: 2,
        CategoryID: 2,
        Picture: '/Assets/data/laptops/08.jpg',
        ModelName: 'Asus ROG Strix G16 G614JU',
        ScreenSize: 16.0,
        OperatingSystem: 'Windows 11',
        ReleaseYear: 2024,
        Description: 'Laptop gaming với màn hình lớn và tản nhiệt hiệu quả',
        Discount: 15000000,
        DiscountStartDate: new Date('2025-02-05T00:00:00Z'),
        DiscountEndDate: new Date('2025-03-05T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-01-20T09:30:00Z'),
        updatedAt: new Date('2025-01-20T09:30:00Z')
      },
      {
        LaptopID: '20353',
        BrandID: 1,
        CategoryID: 4,
        Picture: '/Assets/data/laptops/09.jpg',
        ModelName: 'Dell Mobile Precision Workstation 3581',
        ScreenSize: 15.6,
        OperatingSystem: 'Windows 11',
        ReleaseYear: 2024,
        Description: 'Laptop workstation chuyên nghiệp cho công việc đồ họa',
        Discount: 16000000,
        DiscountStartDate: new Date('2025-02-01T00:00:00Z'),
        DiscountEndDate: new Date('2025-03-01T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-01-22T11:15:00Z'),
        updatedAt: new Date('2025-01-22T11:15:00Z')
      },
      {
        LaptopID: '20388',
        BrandID: 3,
        CategoryID: 1,
        Picture: '/Assets/data/laptops/10.jpg',
        ModelName: 'Lenovo LOQ Essential 15IAX9E',
        ScreenSize: 15.6,
        OperatingSystem: 'Windows 11',
        ReleaseYear: 2024,
        Description: 'Laptop giá rẻ với hiệu năng ổn định',
        Discount: 17000000,
        DiscountStartDate: new Date('2025-01-15T00:00:00Z'),
        DiscountEndDate: new Date('2025-02-15T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-01-23T14:40:00Z'),
        updatedAt: new Date('2025-01-23T14:40:00Z')
      },
      {
        LaptopID: '20577',
        BrandID: 4,
        CategoryID: 2,
        Picture: '/Assets/data/laptops/11.jpg',
        ModelName: 'HP Victus 16 R0223TX',
        ScreenSize: 16.0,
        OperatingSystem: 'Windows 11',
        ReleaseYear: 2024,
        Description: 'Laptop gaming tầm trung với thiết kế hiện đại',
        Discount: 27000000,
        DiscountStartDate: new Date('2025-02-10T00:00:00Z'),
        DiscountEndDate: new Date('2025-03-10T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-01-25T10:20:00Z'),
        updatedAt: new Date('2025-01-25T10:20:00Z')
      },
      {
        LaptopID: '20626',
        BrandID: 9,
        CategoryID: 2,
        Picture: '/Assets/data/laptops/12.jpg',
        ModelName: 'Gigabyte AORUS 16X 9KG',
        ScreenSize: 16.0,
        OperatingSystem: 'Windows 11',
        ReleaseYear: 2024,
        Description: 'Laptop gaming cao cấp với màn hình 16 inch',
        Discount: 21000000,
        DiscountStartDate: new Date('2025-02-12T00:00:00Z'),
        DiscountEndDate: new Date('2025-03-12T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-01-26T13:45:00Z'),
        updatedAt: new Date('2025-01-26T13:45:00Z')
      },
      {
        LaptopID: '20627',
        BrandID: 9,
        CategoryID: 2,
        Picture: '/Assets/data/laptops/13.jpg',
        ModelName: 'Gigabyte G6 KF',
        ScreenSize: 15.6,
        OperatingSystem: 'Windows 11',
        ReleaseYear: 2024,
        Description: 'Laptop gaming với giá thành hợp lý',
        Discount: 22000000,
        DiscountStartDate: new Date('2025-01-20T00:00:00Z'),
        DiscountEndDate: new Date('2025-02-20T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-01-27T09:10:00Z'),
        updatedAt: new Date('2025-01-27T09:10:00Z')
      },
      {
        LaptopID: '20692',
        BrandID: 8,
        CategoryID: 2,
        Picture: '/Assets/data/laptops/14.jpg',
        ModelName: 'MSI Titan 18 HX AI A2XWJG 034VN Dragon Edition',
        ScreenSize: 18.0,
        OperatingSystem: 'Windows 11',
        ReleaseYear: 2024,
        Description: 'Laptop gaming cao cấp phiên bản đặc biệt Dragon Edition',
        Discount: 22200000,
        DiscountStartDate: new Date('2025-02-05T00:00:00Z'),
        DiscountEndDate: new Date('2025-03-05T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-01-30T11:30:00Z'),
        updatedAt: new Date('2025-01-30T11:30:00Z')
      },
      {
        LaptopID: '20724',
        BrandID: 2,
        CategoryID: 2,
        Picture: '/Assets/data/laptops/15.jpg',
        ModelName: 'Asus ROG Zephyrus G16 GA605',
        ScreenSize: 16.0,
        OperatingSystem: 'Windows 11',
        ReleaseYear: 2024,
        Description: 'Laptop gaming mỏng nhẹ với hiệu năng mạnh mẽ',
        Discount: 21200000,
        DiscountStartDate: new Date('2025-02-01T00:00:00Z'),
        DiscountEndDate: new Date('2025-03-01T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-01-31T14:45:00Z'),
        updatedAt: new Date('2025-01-31T14:45:00Z')
      },
      {
        LaptopID: '20993',
        BrandID: 3,
        CategoryID: 1,
        Picture: '/Assets/data/laptops/16.jpg',
        ModelName: 'Lenovo Ideapad 5 Xiaoxin Pro 14 AMD',
        ScreenSize: 14.0,
        OperatingSystem: 'Windows 11',
        ReleaseYear: 2024,
        Description: 'Laptop văn phòng với chip AMD tiết kiệm điện',
        Discount: 21600000,
        DiscountStartDate: new Date('2025-01-25T00:00:00Z'),
        DiscountEndDate: new Date('2025-02-25T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-02-01T09:30:00Z'),
        updatedAt: new Date('2025-02-01T09:30:00Z')
      },
      {
        LaptopID: '21037',
        BrandID: 4,
        CategoryID: 5,
        Picture: '/Assets/data/laptops/17.jpg',
        ModelName: 'HP Omen Transcend 14',
        ScreenSize: 14.0,
        OperatingSystem: 'Windows 11',
        ReleaseYear: 2024,
        Description: 'Laptop gaming cao cấp nhỏ gọn của HP',
        Discount: 10000008,
        DiscountStartDate: new Date('2025-02-10T00:00:00Z'),
        DiscountEndDate: new Date('2025-03-10T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-02-02T10:15:00Z'),
        updatedAt: new Date('2025-02-02T10:15:00Z')
      },
      {
        LaptopID: '21042',
        BrandID: 7,
        CategoryID: 5,
        Picture: '/Assets/data/laptops/18.jpg',
        ModelName: 'Acer Predator Helios Neo 14 PHN14 51',
        ScreenSize: 14.0,
        OperatingSystem: 'Windows 11',
        ReleaseYear: 2024,
        Description: 'Laptop gaming nhỏ gọn với thiết kế độc đáo',
        Discount: 21800000,
        DiscountStartDate: new Date('2025-02-15T00:00:00Z'),
        DiscountEndDate: new Date('2025-03-15T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-02-04T13:20:00Z'),
        updatedAt: new Date('2025-02-04T13:20:00Z')
      },
      {
        LaptopID: '21056',
        BrandID: 4,
        CategoryID: 2,
        Picture: '/Assets/data/laptops/19.jpg',
        ModelName: 'HP Victus 16 R1190TX',
        ScreenSize: 16.0,
        OperatingSystem: 'Windows 11',
        ReleaseYear: 2024,
        Description: 'Laptop gaming tầm trung với màn hình lớn',
        Discount: 21800000,
        DiscountStartDate: new Date('2025-02-01T00:00:00Z'),
        DiscountEndDate: new Date('2025-03-01T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-02-05T09:45:00Z'),
        updatedAt: new Date('2025-02-05T09:45:00Z')
      },
      {
        LaptopID: '21108',
        BrandID: 7,
        CategoryID: 5,
        Picture: '/Assets/data/laptops/20.jpg',
        ModelName: 'Acer Predator Helios Neo 16S AI',
        ScreenSize: 16.0,
        OperatingSystem: 'Windows 11',
        ReleaseYear: 2024,
        Description: 'Laptop gaming cao cấp tích hợp AI',
        Discount: 23800000,
        DiscountStartDate: new Date('2025-02-08T00:00:00Z'),
        DiscountEndDate: new Date('2025-03-08T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-02-06T11:30:00Z'),
        updatedAt: new Date('2025-02-06T11:30:00Z')
      },
      {
        LaptopID: '21171',
        BrandID: 3,
        CategoryID: 3,
        Picture: '/Assets/data/laptops/21.jpg',
        ModelName: 'Lenovo Yoga Slim 9 14ILL10',
        ScreenSize: 14.0,
        OperatingSystem: 'Windows 11',
        ReleaseYear: 2024,
        Description: 'Laptop cao cấp siêu mỏng nhẹ',
        Discount: 23800000,
        DiscountStartDate: new Date('2025-02-03T00:00:00Z'),
        DiscountEndDate: new Date('2025-03-03T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-02-08T09:15:00Z'),
        updatedAt: new Date('2025-02-08T09:15:00Z')
      },
      {
        LaptopID: '21175',
        BrandID: 3,
        CategoryID: 3,
        Picture: '/Assets/data/laptops/22.jpg',
        ModelName: 'Lenovo Yoga Slim 7 14ILL10',
        ScreenSize: 14.0,
        OperatingSystem: 'Windows 11',
        ReleaseYear: 2024,
        Description: 'Laptop mỏng nhẹ với hiệu năng ổn định',
        Discount: 13800000,
        DiscountStartDate: new Date('2025-01-25T00:00:00Z'),
        DiscountEndDate: new Date('2025-02-25T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-02-09T14:20:00Z'),
        updatedAt: new Date('2025-02-09T14:20:00Z')
      },
      {
        LaptopID: '21183',
        BrandID: 3,
        CategoryID: 2,
        Picture: '/Assets/data/laptops/23.jpg',
        ModelName: 'Lenovo Legion 5 R7000 APH9',
        ScreenSize: 15.6,
        OperatingSystem: 'Windows 11',
        ReleaseYear: 2024,
        Description: 'Laptop gaming với chip AMD mạnh mẽ',
        Discount: 13800000,
        DiscountStartDate: new Date('2025-02-12T00:00:00Z'),
        DiscountEndDate: new Date('2025-03-12T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-02-10T10:00:00Z'),
        updatedAt: new Date('2025-02-10T10:00:00Z')
      },
      {
        LaptopID: '21253',
        BrandID: 8,
        CategoryID: 2,
        Picture: '/Assets/data/laptops/24.jpg',
        ModelName: 'MSI Titan 18 HX AI A2XW',
        ScreenSize: 18.0,
        OperatingSystem: 'Windows 11',
        ReleaseYear: 2024,
        Description: 'Laptop gaming cao cấp với màn hình 18 inch',
        Discount: 10000008,
        DiscountStartDate: new Date('2025-02-18T00:00:00Z'),
        DiscountEndDate: new Date('2025-03-18T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-02-11T13:10:00Z'),
        updatedAt: new Date('2025-02-11T13:10:00Z')
      },
      {
        LaptopID: '21302',
        BrandID: 8,
        CategoryID: 2,
        Picture: '/Assets/data/laptops/25.jpg',
        ModelName: 'MSI Raider GE78 HX AI A2XW',
        ScreenSize: 17.3,
        OperatingSystem: 'Windows 11',
        ReleaseYear: 2024,
        Description: 'Laptop gaming cao cấp với công nghệ AI',
        Discount: 14900000,
        DiscountStartDate: new Date('2025-01-20T00:00:00Z'),
        DiscountEndDate: new Date('2025-02-20T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-02-12T09:30:00Z'),
        updatedAt: new Date('2025-02-12T09:30:00Z')
      },
      {
        LaptopID: '21310',
        BrandID: 8,
        CategoryID: 2,
        Picture: '/Assets/data/laptops/26.jpg',
        ModelName: 'MSI Katana A15 AI B8V',
        ScreenSize: 15.6,
        OperatingSystem: 'Windows 11',
        ReleaseYear: 2024,
        Description: 'Laptop gaming tầm trung với thiết kế gaming',
        Discount: 14900000,
        DiscountStartDate: new Date('2025-02-05T00:00:00Z'),
        DiscountEndDate: new Date('2025-03-05T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-02-14T11:45:00Z'),
        updatedAt: new Date('2025-02-14T11:45:00Z')
      },
      {
        LaptopID: '21330',
        BrandID: 1,
        CategoryID: 2,
        Picture: '/Assets/data/laptops/27.jpg',
        ModelName: 'Dell Gaming G15 5530 i7H165W11GR4060',
        ScreenSize: 15.6,
        OperatingSystem: 'Windows 11',
        ReleaseYear: 2024,
        Description: 'Laptop gaming Dell với thiết kế bền bỉ',
        Discount: 10000015,
        DiscountStartDate: new Date('2025-02-01T00:00:00Z'),
        DiscountEndDate: new Date('2025-03-01T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-02-15T14:30:00Z'),
        updatedAt: new Date('2025-02-15T14:30:00Z')
      },
      {
        LaptopID: '21340',
        BrandID: 1,
        CategoryID: 3,
        Picture: '/Assets/data/laptops/28.jpg',
        ModelName: 'Dell Latitude 3450',
        ScreenSize: 14.0,
        OperatingSystem: 'Windows 11',
        ReleaseYear: 2024,
        Description: 'Laptop doanh nhân với độ bền cao',
        Discount: 10000012,
        DiscountStartDate: new Date('2025-01-25T00:00:00Z'),
        DiscountEndDate: new Date('2025-02-25T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-02-16T10:20:00Z'),
        updatedAt: new Date('2025-02-16T10:20:00Z')
      },
      {
        LaptopID: '21345',
        BrandID: 3,
        CategoryID: 3,
        Picture: '/Assets/data/laptops/29.jpg',
        ModelName: 'Lenovo ThinkPad T14s Gen 5',
        ScreenSize: 14.0,
        OperatingSystem: 'Windows 11',
        ReleaseYear: 2024,
        Description: 'Laptop doanh nhân cao cấp với bảo mật tốt',
        Discount: 12000000,
        DiscountStartDate: new Date('2025-02-15T00:00:00Z'),
        DiscountEndDate: new Date('2025-03-15T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-02-18T09:45:00Z'),
        updatedAt: new Date('2025-02-18T09:45:00Z')
      },
      {
        LaptopID: '21355',
        BrandID: 10,
        CategoryID: 3,
        Picture: '/Assets/data/laptops/30.jpg',
        ModelName: 'MacBook Pro 2024 16 inch',
        ScreenSize: 16.0,
        OperatingSystem: 'macOS Sequoia',
        ReleaseYear: 2024,
        Description: 'Laptop cao cấp với chip Apple M4 Pro mạnh mẽ, màn hình Mini-LED XDR',
        Discount: 25000000,
        DiscountStartDate: new Date('2025-02-25T00:00:00Z'),
        DiscountEndDate: new Date('2025-03-25T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-02-20T11:30:00Z'),
        updatedAt: new Date('2025-02-20T11:30:00Z')
      },
      {
        LaptopID: '21360',
        BrandID: 10,
        CategoryID: 3,
        Picture: '/Assets/data/laptops/31.jpg',
        ModelName: 'MacBook Pro 2024 14 inch',
        ScreenSize: 14.0,
        OperatingSystem: 'macOS Sequoia',
        ReleaseYear: 2024,
        Description: 'Laptop cao cấp với chip Apple M4 Max siêu mạnh, màn hình Mini-LED XDR',
        Discount: 30000000,
        DiscountStartDate: new Date('2025-02-25T00:00:00Z'),
        DiscountEndDate: new Date('2025-03-25T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-02-22T14:15:00Z'),
        updatedAt: new Date('2025-02-22T14:15:00Z')
      },
      {
        LaptopID: '21365',
        BrandID: 2,
        CategoryID: 3,
        Picture: '/Assets/data/laptops/32.jpg',
        ModelName: 'Asus Zenbook Duo OLED UX8406MA-PZ142W',
        ScreenSize: 14.0,
        OperatingSystem: 'Windows 11',
        ReleaseYear: 2024,
        Description: 'Laptop cao cấp với màn hình OLED kép, CPU Core Ultra 9 185H và 32GB RAM',
        Discount: 7000000,
        DiscountStartDate: new Date('2025-02-28T00:00:00Z'),
        DiscountEndDate: new Date('2025-03-28T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-02-24T10:15:00Z'),
        updatedAt: new Date('2025-02-24T10:15:00Z')
      },
      {
        LaptopID: '21370',
        BrandID: 1,
        CategoryID: 2,
        Picture: '/Assets/data/laptops/33.jpg',
        ModelName: 'Dell Alienware x17 R2 Gaming',
        ScreenSize: 17.3,
        OperatingSystem: 'Windows 11',
        ReleaseYear: 2024,
        Description: 'Laptop gaming cao cấp với Intel Core i9-12900HK và NVIDIA GeForce RTX 3080 Ti',
        Discount: 15000000,
        DiscountStartDate: new Date('2025-03-01T00:00:00Z'),
        DiscountEndDate: new Date('2025-03-30T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-02-25T11:30:00Z'),
        updatedAt: new Date('2025-02-25T11:30:00Z')
      },
      {
        LaptopID: '21375',
        BrandID: 11,
        CategoryID: 2,
        Picture: '/Assets/data/laptops/34.jpg',
        ModelName: 'Razer Blade 16 (2023)',
        ScreenSize: 16.0,
        OperatingSystem: 'Windows 11',
        ReleaseYear: 2023,
        Description: 'Laptop gaming cao cấp với Intel Core i9 13950HX và NVIDIA GeForce RTX 4060',
        Discount: 10500000,
        DiscountStartDate: new Date('2025-03-05T00:00:00Z'),
        DiscountEndDate: new Date('2025-04-05T23:59:59Z'),
        IsDeleted: false,
        createdAt: new Date('2025-02-26T14:45:00Z'),
        updatedAt: new Date('2025-02-26T14:45:00Z')
      }
    ], {});
  },

  async down (queryInterface, Sequelize) {
    await queryInterface.bulkDelete('Laptops', null, {});
  }
};
